...
export default {
  ...
  mounted: function(){
    ...
    this.date.push(this.info.medicalOrder.startDate);
    this.date.push(this.info.medicalOrder.endDate);
    this.projectList[0].project = this.info.medicalOrder.projectList;
  },
  ...
  methods:{
    ...
    onSubmit() {
      ...
      axios.post('http://localhost:8018/drug/check', this.info).then(function(resp){
        ...
        let routeData = _this.$router.resolve({ path: '/results', query: {result: JSON.stringify(_this.result), basicInfo:JSON.stringify(_this.basicInfo)}});
